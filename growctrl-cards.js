...
`,en={ok:{bg:"rgba(123,232,168,.14)",color:h.ok,label:"Alles OK"},info:{bg:"rgba(154,200,255,.14)",color:h.info,label:"Info"},warning:{bg:"rgba(255,206,122,.14)",color:h.warn,label:"Warnung"},critical:{bg:"rgba(255,157,157,.16)",color:h.crit,label:"Kritisch"},none:{bg:"rgba(133,153,140,.14)",color:"#85998C",label:"Inaktiv"}};var pi="4d72-4461726b-766f6964",Te=()=>pi,Pe,xt=null;function vt(a){if(a?.states===Pe&&xt)return xt;let e=new Map,t=new Set,i={};for(let[n,s]of Object.entries(a?.states??{})){let r=s?.attributes;if(!r?.growctrl_role||!r?.growctrl_tent)continue;let l=String(r.growctrl_tent),c=String(r.growctrl_station??"zelt");e.set(`${l}::${c}::${r.growctrl_role}`,n),c==="zelt"?t.add(l):(i[l]??=new Set).add(c)}return Pe=a?.states,xt={tents:[...t].sort(),stations:Object.fromEntries(Object.entries(i).map(([n,s])=>[n,[...s].sort()])),byRole:e},xt}var M=(a,e,t,i)=>vt(a).byRole.get(`${e}::${t}::${i}`);var z=class extends I{constructor(){super(...arguments);this._config={};this._label=t=>t.label??t.name}static{this.properties={hass:{attribute:!1},_config:{state:!0}}}static{this.styles=rt`
  ...
  `}setConfig(t){this._config={...t}}_fire(t){this._config=t,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:t},bubbles:!0,composed:!0}))}form(t){return o`<ha-form .hass=${this.hass} .data=${this._config} .schema=${t}
    ...
    ${Array.from({length:k+1},(w,C)=>{let b=C/k,F=Y+b*(n-Y-et),O=m-b*(m-d),B=x-b*(x-f);return X`
    ...
    ${Array.from({length:L+1},(w,C)=>{let b=C/L,F=V+b*(i-V-l),O=C===L?"jetzt":`-${Math.round(A*(1-b))}h`;return X`
    ...
    ${a.map((w,C)=>{if(w.data.length<2)return u;let b=w.axis==="right"?v:$,F=w.data.map((ut,ht)=>[Number(y(ht,w.data.length).toFixed(1)),Number(b(ut).toFixed(1))]),O=Ie(F),B=F[F.length-1][0],nt=F[F.length-1][1];return X`
        <defs>
    ...
    <circle cx="${m}" cy="${f}" r="2.6" fill="${e}"/>
      ...
      <button class="gc dd-btn" aria-haspopup="listbox" aria-expanded=${this._phase} @click=${()=>{this._phase=!this._phase}}>
        <span class="pdot ${He[i]}" style="${i==="Auto"?"background:var(--acc);color:var(--acc)":""}"></span>${this.t(i)}
    ...
    </div>`}phaseDropdown(t,i){let n=E(this.st(this.e("age"))),s=this.hass.states[this.e("stage")]?.attributes?.phasen_bis??{},r=c=>{let p=s[c];return typeof p=="number"?`${this.t("bis Tag")} ${p}`:c==="Aus"||c==="Trocknung"?this.t(bi[c]??""):""},l=n!==null?`${this.t("Tag")} ${n}`:r(t);return o`<div class="dd ${this._phase?"open":""}">
      <button class="gc dd-btn" aria-haspopup="listbox" aria-expanded=${this._phase}
        @click=${()=>{this._phase=!this._phase,this._plant=!1}}>
        <span class="pdot" style="background:${i.color};color:${i.color}"></span>${this.t(Ve[t]??t)}
        <span class="hint">${l}</span>
        <ha-icon icon="mdi:chevron-down" style="--mdc-icon-size:16px;color:var(--tx-3);transition:transform .2s;${this._phase?"transform:rotate(180deg)":""}"></ha-icon>
      </button>
      ${this._phase?o`<div class="dd-menu" role="listbox">
        ${fi.map(c=>o`<button class="gc dd-it" role="option" aria-selected=${c===t}
          @click=${()=>{this._select(this.e("stage"),c),this._phase=!1}}>
          <span class="pdot ${xi[c]}"></span>${this.t(Ve[c]??c)}<span class="hint">${r(c)}</span></button>`)}
      </div>`:u}
      ...
      <div class="acts">
        ${t.map(i=>{let n=this.isOn(i.entity),s=i.kind??"",r=i.icon??vi[s]??"mdi:power",l=i.name??this.friendly(i.entity);return o`<button class="gc act ${n?"on":""} ${n&&s?s:""}" style="position:relative"
      ...
      </div>`}todoBlock(){let t=this._config,i=M(this.hass,t.tent,"zelt","todo")??U("todo",t.tent,"aufgaben",t.overrides),n=this.hass.states[i]?.attributes?.aufgaben_offen??[],s=`${t.station}: `,r=n.filter(c=>c.text.startsWith(s)).map(c=>({uid:c.uid,text:c.text.slice(s.length)}));if(!r.length)return u;let l=this.expOpen("todo")?r:r.slice(0,1);return o`${this.expHeader("todo","Aufgaben")}
      <div style="display:flex;flex-direction:column;gap:5px">
      ...
      ${[...i.entries()].map(([n,s])=>o`
        ...
        <div class="acts" style="grid-template-columns:repeat(${t},1fr); ${n?"":"margin-top:4px"}">
          ${s.map(r=>{let l=this.isOn(r.entity),c=r.name??this.friendly(r.entity),p=r.kind??"",d=p==="light"||p==="heat"||p==="water"?p:"",m=r.icon??this.hass.states[r.entity]?.attributes?.icon??_i[p]??wi[r.entity.split(".")[0]]??"mdi:power";return o`<button class="gc act ${l?"on":""} ${l?d:""}" style="position:relative"
      ...
      ${(t.charts??[]).map(s=>{let r=E(this.st(s.entity)),l=this.bad(r,s),c=s.name??this.friendly(s.entity);return o`<div style="margin-top:14px">
    ...
    </div>`}};customElements.define("growctrl-sensors-card",Xt);var Ci=[g.text("title","\u{1F3F7}\uFE0F Titel"),g.num("limit","\u{1F4CB} Max. Zeilen",3,50),g.select("min_level","\u{1F50D} Anzeige",[{value:"alle",label:"Alle Ereignisse"},{value:"warnung",label:"Nur Warnungen/Fehler"},{value:"info",label:"Nur Infos"}])],Ei=[g.entity("entity","\u{1F4DC} Letztes-Ereignis-Sensor","sensor"),g.text("name","\u270F\uFE0F Label (optional)")],Yt=class extends z{render(){return o`${this.form(Ci)}
      ${this.list({key:"sources",rowSchema:Ei,title:"\u{1F4E1} Quellen",addLabel:"Quelle hinzuf\xFCgen",newItem:()=>({entity:""})})}
      ${this.styleSection()}
    ...
    </div>`}};customElements.define("growctrl-status-card",Qt);var zi=[g.text("title","\u{1F3F7}\uFE0F Titel (optional)"),g.text("logo","\u{1F5BC}\uFE0F Logo-URL (z.B. /local/growctrl/logo.png)"),g.bool("show_chart","\u{1F4C8} 24h-Chart zus\xE4tzlich zum Zonen-Balken"),g.num("hours","\u23F1\uFE0F Chart-Zeitraum (h)",1,168)],Jt=class extends z{render(){let e=[this.stationSelect(this._config?.tent),g.text("name","\u270F\uFE0F Anzeigename (optional)")];return o`${this.form([this.tentSelect("tent","\u{1F3D5}\uFE0F Zelt"),...zi])}
      ...
      <div class="hint">Die Hero-Karte ist die Zelt-\u00dcbersicht: Klima-Werte, VPD-Skala und das
        Informationssystem. Die gelisteten <b>Stationen</b> liefern die Ereigniszeilen darunter.</div>`}};customElements.define("growctrl-hero-editor",Jt);var Ue=[{w:20,col:"#6E97DE",lbl:"zu feucht"},{w:20,col:"#58E0A5",lbl:"Seedling"},{w:20,col:"#2FB36C",lbl:"Veg"},{w:20,col:"#E5B567",lbl:"Bloom"},{w:20,col:"#D4726F",lbl:"zu trocken"}],te=2,ee=class extends S{constructor(){super(...arguments);this._logoErr=!1;this._hist=[]}static{this.styles=T}static{this.properties={...S.properties,_hist:{state:!0},_logoErr:{state:!0}}}validateConfig(t){if(!t.tent)throw new Error("growctrl-hero-card: 'tent' ist Pflicht (Zelt-Name aus der Integration).")}static getConfigElement(){return document.createElement("growctrl-hero-editor")}static getStubConfig(){return{tent:"gross",stations:[{station:"main1"}]}}te(t){let[i,n,s]=it[t],r=this._config;return r.overrides?.[n]??M(this.hass,r.tent,"zelt",s)??U(i,r.tent,n,r.overrides)}connectedCallback(){super.connectedCallback(),this._load(),this._timer=window.setInterval(()=>this._load(),5*6e4)}disconnectedCallback(){super.disconnectedCallback(),this._timer&&clearInterval(this._timer)}async _load(){if(!this.hass){setTimeout(()=>this._load(),1e3);return}this._hist=await R(this.hass,this.te("vpd"),this._config.hours??24)}tglBtn(t,i,n){return o`<button class="gc tgl ${n?"on":""}" @click=${()=>this.confirmToggle(t,i)}>
      <span class="sw"></span> ${i}</button>`}render(){let t=this._config;if(!this.hass)return u;let i=this.hass.states[this.te("vpd")],n=!i&&!this.hass.states[this.te("enabled")],s=E(i?.state)??(n?.76:null),r=i?.attributes?.temp??(n?21.5:null),l=i?.attributes?.rh??(n?61:null),c=i?.attributes?.phase_effektiv??"",p=i?.attributes?.sollwerte,d=this.isOn(this.te("enabled"))||n,m=this.isOn(this.te("climate")),f=this.hass.states[this.te("status")],x=f?.attributes?.probleme??[],y=(t.stations??[]).map(b=>{let F=this.hass.states[M(this.hass,t.tent,b.station,"last_event")??Z("sensor",t.tent,b.station,"letztes_ereignis",t.overrides)],O=this.hass.states[M(this.hass,t.tent,b.station,"light_rest")??Z("sensor",t.tent,b.station,"licht_restzeit",t.overrides)],B=F?.attributes?.schweregrad??"ok";return{name:b.name??b.station,text:F?.state??"\u2013",level:B,lightText:O?.attributes?.text??(O?.state?`${this.t("Licht")} ${this.t(String(O.attributes?.zustand??""))}`:""),on:O?.attributes?.zustand==="an",ent:M(this.hass,t.tent,b.station,"last_event")??Z("sensor",t.tent,b.station,"letztes_ereignis",t.overrides)}}),$=b=>b==="warning"||b==="critical",v=tt([(f?.state??"").toLowerCase()==="problem"?"warning":"ok",...y.map(b=>$(b.level)?b.level:"ok")]),k=[...x.map(b=>({label:b,level:"warning"})),...y.filter(b=>$(b.level)).map(b=>({label:`${b.name}: ${b.text}`,level:b.level}))],_=s!==null&&p&&s>=p.vpd_min&&s<=p.vpd_max,A=s!==null?Math.min(100,Math.max(0,s/te*100)):null,L=this.isOn(this.te("stale")),w=i?.attributes?.temp_entity||this.te("vpd"),C=i?.attributes?.rh_entity||this.te("vpd");return o`<div class="card ${t.style?.glass?"glass":""}" data-level=${v} style="${P(t.style)};position:relative">
      <div class="hd">
    ...
    </div>`}};customElements.define("growctrl-checkup-card",ne);var Fi=[g.text("title","\u{1F3F7}\uFE0F Titel"),g.entity("entity","\u{1F4A7} F\xFCllstand-Sensor (%) (Pflicht)","sensor"),g.num("min","\u26A0\uFE0F Mindeststand (%)",0,100),g.num("volume_l","\u{1FAA3} Tankvolumen (Liter, optional)",0,2e3)],se=class extends z{render(){return o`${this.form(Fi)}${this.styleSection()}
    ...
    </div>`}};customElements.define("growctrl-tank-card",re);var Pi=[g.text("title","\u{1F3F7}\uFE0F Titel"),g.num("hours","\u23F1\uFE0F Zeitraum (h)",1,168),g.num("height","\u{1F4CF} Diagrammh\xF6he (px)",80,300)],Ti=[g.entity("entity","\u{1F4C8} Sensor","sensor"),g.text("name","\u270F\uFE0F Anzeigename (optional)"),g.select("axis","\u{1F4CA} Achse",[{value:"left",label:"Links (Standard)"},{value:"right",label:"Rechts (zweite Achse)"}]),g.text("color","\u{1F3A8} Farbe (optional, z.B. #FF9F5A)")],ae=class extends z{render(){return o`${this.form(Pi)}
      ${this.list({key:"sensors",rowSchema:Ti,title:"\u{1F4C9} Serien",addLabel:"Sensor hinzuf\xFCgen",newItem:()=>({entity:""})})}
      ${this.styleSection()}
    ...
    </div>`}};customElements.define("growctrl-metric-card",ce);var Oi="4.2.0",Ri=[{type:"growctrl-tent-card",name:"GROWCTRL Tent",description:"Zelt-Hero: Klima-KPIs, VPD-Skala, Status-Ampel"},{type:"growctrl-station-card",name:"GROWCTRL Station",description:"Station: Licht-/Pumpenbalken, Stage, Auto, Konfiguration"},{type:"growctrl-controls-card",name:"GROWCTRL Controls",description:"Aktoren-Raster mit Gruppen und Bestaetigung"},{type:"growctrl-sensors-card",name:"GROWCTRL Sensors",description:"Sensor-KPIs mit Sollbereich-Ampel"},{type:"growctrl-status-card",name:"GROWCTRL Status",description:"Ereignisprotokoll der Integration mit Schweregrad-Ampel"},{type:"growctrl-hero-card",name:"GROWCTRL Hero",description:"Globale Steuerung, Klima-KPIs, VPD-Chart, Informationssystem"},{type:"growctrl-checkup-card",name:"GROWCTRL Checkup",description:"Checkup-Matrix: Licht/Pumpe/Klima/Status je Station"},{type:"growctrl-tank-card",name:"GROWCTRL Tank",description:"DWC-Fuellstand (vertikaler Tank) mit Warnstufe"},{type:"growctrl-history-card",name:"GROWCTRL History",description:"24h-Diagramm (z.B. Temperatur + Luftfeuchte)"},{type:"growctrl-metric-card",name:"GROWCTRL Metric",description:"EC/pH gross mit Chart und Sollbereich"}];window.customCards=window.customCards??[];window.__gcEpoch=Te();Ri.forEach(a=>window.customCards.push({...a,preview:!0,documentationURL:"https://github.com/MrDarkvoid/growctrl"}));console.info(`%c GROWCTRL Cards %c v${Oi} `,"background:#1D9E75;color:#fff;font-weight:700","background:#0F6E56;color:#fff");
