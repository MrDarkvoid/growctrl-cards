...
`,on={ok:{bg:"rgba(123,232,168,.14)",color:h.ok,label:"Alles OK"},info:{bg:"rgba(154,200,255,.14)",color:h.info,label:"Info"},warning:{bg:"rgba(255,206,122,.14)",color:h.warn,label:"Warnung"},critical:{bg:"rgba(255,157,157,.16)",color:h.crit,label:"Kritisch"},none:{bg:"rgba(133,153,140,.14)",color:"#85998C",label:"Inaktiv"}};var hi="4d72-4461726b-766f6964",Re=()=>hi,Me,xt=null;function vt(a){if(a?.states===Me&&xt)return xt;let e=new Map,t=new Set,i={};for(let[n,s]of Object.entries(a?.states??{})){let r=s?.attributes;if(!r?.growctrl_role||!r?.growctrl_tent)continue;let l=String(r.growctrl_tent),c=String(r.growctrl_station??"zelt");e.set(`${l}::${c}::${r.growctrl_role}`,n),c==="zelt"?t.add(l):(i[l]??=new Set).add(c)}return Me=a?.states,xt={tents:[...t].sort(),stations:Object.fromEntries(Object.entries(i).map(([n,s])=>[n,[...s].sort()])),byRole:e},xt}var R=(a,e,t,i)=>vt(a).byRole.get(`${e}::${t}::${i}`);var E=class extends I{constructor(){super(...arguments);this._config={};this._label=t=>t.label??t.name}static{this.properties={hass:{attribute:!1},_config:{state:!0}}}static{this.styles=rt`
  ...
  `}setConfig(t){this._config={...t}}_fire(t){this._config=t,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:t},bubbles:!0,composed:!0}))}form(t){return o`<ha-form .hass=${this.hass} .data=${this._config} .schema=${t}
    ...
    ${Array.from({length:k+1},(w,C)=>{let b=C/k,P=Y+b*(n-Y-et),O=m-b*(m-d),B=x-b*(x-f);return X`
        <line x1="${V}" y1="${P}" x2="${i-l}" y2="${P}"
          stroke="rgba(255,255,255,.10)" stroke-width="1"/>
        <text x="${V-4}" y="${P+3}" text-anchor="end" class="c-axl"
          font-size="9.5" fill="rgba(255,255,255,.68)">${_(O)}</text>
        ${r?X`<text x="${i-l+5}" y="${P+3}" text-anchor="start" class="c-axl"
          font-size="9.5" fill="rgba(255,255,255,.55)">${_(B)}</text>`:u}`})}
    ${Array.from({length:F+1},(w,C)=>{let b=C/F,P=V+b*(i-V-l),O=C===F?"jetzt":`-${Math.round(A*(1-b))}h`;return X`
        <line x1="${P}" y1="${Y}" x2="${P}" y2="${n-et}" stroke="rgba(255,255,255,.05)" stroke-width="1"/>
        <text x="${P}" y="${n-4}" text-anchor="${C===0?"start":C===F?"end":"middle"}"
          class="c-axl" font-size="9" fill="rgba(255,255,255,.5)">${O}</text>`})}
    ${a.map((w,C)=>{if(w.data.length<2)return u;let b=w.axis==="right"?v:$,P=w.data.map((ut,ht)=>[Number(y(ht,w.data.length).toFixed(1)),Number(b(ut).toFixed(1))]),O=Ne(P),B=P[P.length-1][0],nt=P[P.length-1][1];return X`
        <defs>
    ...
    <circle cx="${m}" cy="${f}" r="2.6" fill="${e}"/>
      ...
      <div style="margin-top:16px">
        <div class="zones">
          ${Ue.map(F=>o`<i style="width:${F.w}%;background:${F.col}"></i>`)}
        ...
        <div class="zlbl">${Ue.map(F=>o`<span style="width:${F.w}%">${this.t(F.lbl)}</span>`)}</div>
      </div>
      ...
      ${x.length?o`<div style="display:flex; flex-wrap:wrap; gap:7px; margin-top:12px">
        ${x.map(F=>o`<span class="pbadge warn"><ha-icon icon="mdi:alert" style="--mdc-icon-size:12px"></ha-icon>${F}</span>`)}</div>`:u}
    ...
    </div>`}phaseDropdown(t,i){let n=z(this.st(this.e("age"))),s=this.hass.states[this.e("stage")]?.attributes?.phasen_bis??{},r=c=>{let p=s[c];return typeof p=="number"?`${this.t("bis Tag")} ${p}`:c==="Aus"||c==="Trocknung"?this.t(yi[c]??""):""},l=n!==null?`${this.t("Tag")} ${n}`:r(t);return o`<div class="dd ${this._phase?"open":""}">
      <button class="gc dd-btn" aria-haspopup="listbox" aria-expanded=${this._phase}
        @click=${()=>{this._phase=!this._phase,this._plant=!1}}>
        <span class="pdot" style="background:${i.color};color:${i.color}"></span>${this.t(Ke[t]??t)}
      ...
      ${this._phase?o`<div class="dd-menu" role="listbox">
        ${vi.map(c=>o`<button class="gc dd-it" role="option" aria-selected=${c===t}
          ...
          <span class="pdot ${$i[c]}"></span>${this.t(Ke[c]??c)}<span class="hint">${r(c)}</span></button>`)}
      </div>`:u}
      ...
      <div style="display:flex;flex-wrap:wrap;gap:12px;padding:2px 2px 0">
        ${t.map(i=>{let n=z(this.st(i.entity)),s=i.anzeige??"wert",r,l=!0;if(s==="zone"){let c=i.ideal??[0,0],p=i.ok??c;r=i.color??(n!==null&&n>=c[0]&&n<=c[1]?h.ok:n!==null&&n>=p[0]&&n<=p[1]?h.warn:h.crit),l=!1}else r=i.color??"var(--tx-3)";return o`<span style="display:inline-flex;align-items:center;gap:5px;font:800 calc(12px * var(--gc-sc,1)) var(--f-num);color:var(--tx-2)">
      ...
      ${c?o`<span class="setrow">
            ...
            <button class="gc stepbtn" title=${this.t("mehr")} @click=${C=>{C.stopPropagation(),y((i??m??0)+d)}}><ha-icon icon="mdi:plus" style="--mdc-icon-size:16px"></ha-icon></button></span>`:o`<span class="ival" style="color:${$}">
            ${A}${i!==null?i:this.st(t.entity)??"\u2013"}<span class="u">${s}</span></span>`}
    </div>`,w=r==="zone"?o`${this.zoneV6(i,t.min??0,t.max??14,k,v)}
          ${(this._spark[t.entity]?.length??0)>1?o`<div class="spark" style="margin-top:7px;opacity:.85">${Dt(this._spark[t.entity],$,this.chartW(74),28)}</div>`:u}`:r==="graph"?o`<div class="spark">${Dt(this._spark[t.entity]??[],$,this.chartW(74),38)}</div>`:u;return o`<div class="ind" style="cursor:pointer" @click=${()=>this.moreInfo(t.entity)}>${F}${w}</div>`}mediaBlock(){let t=this._config,i=t.media_height??180;if(t.image_url)return o`<div class="seclbl">${this.t("Livestream")}</div>
      ...
      <div style="display:flex;flex-direction:column;gap:5px">
        ${s.map(r=>o`<button class="gc" @click=${()=>this.moreInfo(this.e("event"))}
          ...
          <span style="width:7px;height:7px;border-radius:50%;flex-shrink:0;background:${Pe[r.level]??h.info}"></span>
      ...
      ${[...i.entries()].map(([n,s])=>o`
        ...
        <div class="acts" style="grid-template-columns:repeat(${t},1fr); ${n?"":"margin-top:4px"}">
          ${s.map(r=>{let l=this.isOn(r.entity),c=r.name??this.friendly(r.entity),p=r.kind??"",d=p==="light"||p==="heat"||p==="water"?p:"",m=r.icon??this.hass.states[r.entity]?.attributes?.icon??Ai[p]??Si[r.entity.split(".")[0]]??"mdi:power";return o`<button class="gc act ${l?"on":""} ${l?d:""}" style="position:relative"
    ...
    </div>`}};customElements.define("growctrl-status-card",Qt);var Fi=[g.text("title","\u{1F3F7}\uFE0F Titel (optional)"),g.text("logo","\u{1F5BC}\uFE0F Logo-URL (z.B. /local/growctrl/logo.png)"),g.bool("show_chart","\u{1F4C8} 24h-Chart zus\xE4tzlich zum Zonen-Balken"),g.num("hours","\u23F1\uFE0F Chart-Zeitraum (h)",1,168)],Jt=class extends E{render(){let e=[this.stationSelect(this._config?.tent),g.text("name","\u270F\uFE0F Anzeigename (optional)")];return o`${this.form([this.tentSelect("tent","\u{1F3D5}\uFE0F Zelt"),...Fi])}
      ...
      <div style="margin-top:16px">
        <div class="zones">
          ${Ze.map(b=>o`<i style="width:${b.w}%;background:${b.col}"></i>`)}
        ...
        <div class="zlbl">${Ze.map(b=>o`<span style="width:${b.w}%">${this.t(b.lbl)}</span>`)}</div>
      </div>
      ...
      ${this.renderConfirm()}
    </div>`}};customElements.define("growctrl-hero-card",ee);var ie=class extends E{render(){let e=this._config.tent,t=[this.tentSelect("tent","\u{1F3D5}\uFE0F Zelt"),g.text("title","\u{1F3F7}\uFE0F Titel"),g.bool("show_sensors","\u{1F9EA} Sensoren-Sektion zeigen (pH/EC/Temp/RH/\u2026, Standard an)"),g.bool("show_controls","\u{1F39B}\uFE0F Steuerungs-Sektion zeigen (Licht/Pumpe/\u2026, Standard an)"),g.bool("show_tent_row","\u{1F3D5}\uFE0F Zelt-Sektion zeigen (Aktiv/Klima/VPD/Status)"),g.text("tent_name","\u270F\uFE0F Name der Zelt-Zeile (optional)")],i=[this.stationSelect(e,"station","\u{1F331} Station (= Pflanze)"),g.text("name","\u270F\uFE0F Anzeigename (optional)")];return o`${this.form(t)}
    ...
    </div>`}};customElements.define("growctrl-metric-card",ce);var Ii=[g.text("title","\u{1F3F7}\uFE0F Titel"),g.num("columns","\u25A6 Spalten",1,4)],Hi=[g.text("label","\u270F\uFE0F Beschriftung"),g.text("icon","\u{1F3A8} Icon (z.B. mdi:lightbulb-off)"),g.text("service","\u26A1 Service (z.B. light.turn_off)"),{name:"data",label:"\u{1F4E6} Service-Daten (optional, YAML/JSON)",selector:{object:{}}},g.bool("confirm","\u2705 Vor dem Ausf\xFChren best\xE4tigen")],pe=class extends E{render(){return o`${this.form(Ii)}
      ${this.list({key:"buttons",rowSchema:Hi,title:"\u26A1 Aktionen (Buttons)",addLabel:"Aktion hinzuf\xFCgen",newItem:()=>({label:"",service:""})})}
      ${this.styleSection()}
      ...
      ${this.renderConfirm()}
    </div>`}};customElements.define("growctrl-actions-card",de);var Ni="4.3.0",Di=[{type:"growctrl-tent-card",name:"GROWCTRL Tent",description:"Zelt-Hero: Klima-KPIs, VPD-Skala, Status-Ampel"},{type:"growctrl-station-card",name:"GROWCTRL Station",description:"Station: Licht-/Pumpenbalken, Stage, Auto, Konfiguration"},{type:"growctrl-controls-card",name:"GROWCTRL Controls",description:"Aktoren-Raster mit Gruppen und Bestaetigung"},{type:"growctrl-sensors-card",name:"GROWCTRL Sensors",description:"Sensor-KPIs mit Sollbereich-Ampel"},{type:"growctrl-status-card",name:"GROWCTRL Status",description:"Ereignisprotokoll der Integration mit Schweregrad-Ampel"},{type:"growctrl-hero-card",name:"GROWCTRL Hero",description:"Globale Steuerung, Klima-KPIs, VPD-Chart, Informationssystem"},{type:"growctrl-checkup-card",name:"GROWCTRL Checkup",description:"Checkup-Matrix: Licht/Pumpe/Klima/Status je Station"},{type:"growctrl-tank-card",name:"GROWCTRL Tank",description:"DWC-Fuellstand (vertikaler Tank) mit Warnstufe"},{type:"growctrl-history-card",name:"GROWCTRL History",description:"24h-Diagramm (z.B. Temperatur + Luftfeuchte)"},{type:"growctrl-metric-card",name:"GROWCTRL Metric",description:"EC/pH gross mit Chart und Sollbereich"},{type:"growctrl-actions-card",name:"GROWCTRL Actions",description:"Schnell-Aktionen: konfigurierbare Buttons, rufen Services auf"}];window.customCards=window.customCards??[];window.__gcEpoch=Re();Di.forEach(a=>window.customCards.push({...a,preview:!0,documentationURL:"https://github.com/MrDarkvoid/growctrl"}));console.info(`%c GROWCTRL Cards %c v${Ni} `,"background:#1D9E75;color:#fff;font-weight:700","background:#0F6E56;color:#fff");
