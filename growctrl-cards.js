...
`,Ji={ok:{bg:"rgba(123,232,168,.14)",color:h.ok,label:"Alles OK"},info:{bg:"rgba(154,200,255,.14)",color:h.info,label:"Info"},warning:{bg:"rgba(255,206,122,.14)",color:h.warn,label:"Warnung"},critical:{bg:"rgba(255,157,157,.16)",color:h.crit,label:"Kritisch"},none:{bg:"rgba(133,153,140,.14)",color:"#85998C",label:"Inaktiv"}};var pi="4d72-4461726b-766f6964",Te=()=>pi,Fe,xt=null;function vt(a){if(a?.states===Fe&&xt)return xt;let e=new Map,t=new Set,i={};for(let[n,s]of Object.entries(a?.states??{})){let r=s?.attributes;if(!r?.growctrl_role||!r?.growctrl_tent)continue;let l=String(r.growctrl_tent),c=String(r.growctrl_station??"zelt");e.set(`${l}::${c}::${r.growctrl_role}`,n),c==="zelt"?t.add(l):(i[l]??=new Set).add(c)}return Fe=a?.states,xt={tents:[...t].sort(),stations:Object.fromEntries(Object.entries(i).map(([n,s])=>[n,[...s].sort()])),byRole:e},xt}var M=(a,e,t,i)=>vt(a).byRole.get(`${e}::${t}::${i}`);var z=class extends B{constructor(){super(...arguments);this._config={};this._label=t=>t.label??t.name}static{this.properties={hass:{attribute:!1},_config:{state:!0}}}static{this.styles=rt`
  ...
  `}setConfig(t){this._config={...t}}_fire(t){this._config=t,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:t},bubbles:!0,composed:!0}))}form(t){return o`<ha-form .hass=${this.hass} .data=${this._config} .schema=${t}
    ...
    ${Array.from({length:_+1},(w,C)=>{let b=C/_,P=Y+b*(n-Y-et),O=g-b*(g-d),R=x-b*(x-f);return X`
        <line x1="${D}" y1="${P}" x2="${i-l}" y2="${P}"
          stroke="rgba(255,255,255,.10)" stroke-width="1"/>
        <text x="${D-4}" y="${P+3}" text-anchor="end" class="c-axl"
          font-size="9.5" fill="rgba(255,255,255,.68)">${k(O)}</text>
        ${r?X`<text x="${i-l+5}" y="${P+3}" text-anchor="start" class="c-axl"
          font-size="9.5" fill="rgba(255,255,255,.55)">${k(R)}</text>`:u}`})}
    ${Array.from({length:L+1},(w,C)=>{let b=C/L,P=D+b*(i-D-l),O=C===L?"jetzt":`-${Math.round(A*(1-b))}h`;return X`
        <line x1="${P}" y1="${Y}" x2="${P}" y2="${n-et}" stroke="rgba(255,255,255,.05)" stroke-width="1"/>
        <text x="${P}" y="${n-4}" text-anchor="${C===0?"start":C===L?"end":"middle"}"
          class="c-axl" font-size="9" fill="rgba(255,255,255,.5)">${O}</text>`})}
    ${a.map((w,C)=>{if(w.data.length<2)return u;let b=w.axis==="right"?v:$,P=w.data.map((ut,ht)=>[Number(y(ht,w.data.length).toFixed(1)),Number(b(ut).toFixed(1))]),O=Ie(P),R=P[P.length-1][0],nt=P[P.length-1][1];return X`
        <defs>
    ...
    <circle cx="${g}" cy="${f}" r="2.6" fill="${e}"/>
      ...
      <div class="acts">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:11px;padding:2px 2px 0">
          ${t.map(i=>{let n=this.isOn(i.entity);return o`<span style="display:inline-flex;align-items:center;gap:6px;font:800 11.5px var(--f-ui);color:var(--tx-2)">
      ...
      <div style="display:flex;flex-wrap:wrap;gap:12px;padding:2px 2px 0">
        ${t.map(i=>{let n=E(this.st(i.entity)),s=i.anzeige??"wert",r,l=!0;if(s==="zone"){let c=i.ideal??[0,0],p=i.ok??c;r=i.color??(n!==null&&n>=c[0]&&n<=c[1]?h.ok:n!==null&&n>=p[0]&&n<=p[1]?h.warn:h.crit),l=!1}else r=i.color??"var(--tx-3)";return o`<span style="display:inline-flex;align-items:center;gap:5px;font:800 12px var(--f-num);color:var(--tx-2)">
            <span style="width:9px;height:9px;border-radius:50%;flex-shrink:0;background:${r};box-shadow:0 0 7px ${r}"></span>
            ${i.icon?o`<ha-icon icon="${i.icon}" style="--mdc-icon-size:13px;color:var(--tx-3)"></ha-icon>`:u}
      ...
      </span>`}sensorInd(t){let i=E(this.st(t.entity)),n=t.name??this.friendly(t.entity),s=this.unit(t.entity),r=t.anzeige??"wert",l=t.entity.split(".")[0],c=l==="number"||l==="input_number",p=this.hass.states[t.entity]?.attributes??{},d=t.step??(Number(p.step)||.1),g=p.min,f=p.max,x=(String(d).split(".")[1]??"").length||1,y=C=>{let b=C;g!==void 0&&(b=Math.max(g,b)),f!==void 0&&(b=Math.min(f,b)),this.hass.callService(l,"set_value",{entity_id:t.entity,value:Number(b.toFixed(x))})},$,v=t.ideal??[0,0],_=t.ok??v;if(r==="zone"){let C=i!==null&&i>=v[0]&&i<=v[1],b=i!==null&&i>=_[0]&&i<=_[1];$=t.color??(C?h.ok:b?h.warn:h.crit)}else r==="graph"?$=t.color??h.water:$=t.color??"rgba(242,247,243,.95)";let k=this.trendArrow(this._spark[t.entity]),A=k?o`<ha-icon icon="${k.icon}" title=${this.t("Trend")}
          style="--mdc-icon-size:${k.size}px;color:${k.color};margin-right:3px;vertical-align:-3px"></ha-icon>`:u,L=o`<div class="ihd">
      ...
      ${t.show_chart===!0&&this._hist.length>1?o`
        <div class="seclbl">VPD · ${t.hours??24}h</div>
        ${W([{data:this._hist,color:k===!1?h.warn:h.ok,fill:!0}],{w:this.chartW(),h:96,band:p?{min:p.vpd_min,max:p.vpd_max}:void 0,grid:3})}`:u}
    ...
    </div>`}};customElements.define("growctrl-tank-card",re);var Li=[m.text("title","\u{1F3F7}\uFE0F Titel"),m.num("hours","\u23F1\uFE0F Zeitraum (h)",1,168),m.num("height","\u{1F4CF} Diagrammh\xF6he (px)",80,300)],Pi=[m.entity("entity","\u{1F4C8} Sensor","sensor"),m.text("name","\u270F\uFE0F Anzeigename (optional)"),m.select("axis","\u{1F4CA} Achse",[{value:"left",label:"Links (Standard)"},{value:"right",label:"Rechts (zweite Achse)"}]),m.text("color","\u{1F3A8} Farbe (optional, z.B. #FF9F5A)")],ae=class extends z{render(){return o`${this.form(Li)}
